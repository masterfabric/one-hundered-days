...
const Toast = (props: ToastProps) => {
  ...
  React.useLayoutEffect(() => {
    ...
    setHeights((heights) => {
      ...
      if (!alreadyExists) {
        return [{ toastId: toast.id, height: newHeight, position: toast.position as any }, ...heights];
      } else {
  ...
  const icon = toast.icon || (toastType && (icons as any)?.[toastType]) || getAsset(toastType as any);
  return (
    <li
      tabIndex={0}
      ref={toastRef}
    ...
    >
      ...
      toast.icon !== null &&
      (icons?.[toastType as keyof typeof icons] !== null || toast.icon) ? (
...
const Toaster = React.forwardRef<HTMLElement, ToasterProps>(function Toaster(props, ref) {
  ...
  const possiblePositions = React.useMemo(() => {
    return Array.from(
     new Set([position].concat(filteredToasts.filter((toast) => toast.position).map((toast) => toast.position) as any)),
    );
